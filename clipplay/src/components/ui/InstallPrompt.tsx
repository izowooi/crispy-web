...
export function InstallPrompt() {
  ...
  if (isIOS && showInstall) {
    return (
      <div className="fixed bottom-4 left-4 right-4 bg-card-bg border border-card-border p-4 rounded-2xl shadow-xl z-50">
      ...
      <div className="fixed bottom-4 left-4 right-4 bg-card-bg border border-card-border p-4 rounded-2xl shadow-xl z-50">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <button
              onClick={handleDismiss}
              className="px-3 py-1.5 text-sm text-foreground/50 hover:text-foreground/70 transition-colors"
            >
              나중에
            </button>
            <button
              onClick={handleInstallClick}
              className="px-4 py-1.5 bg-primary hover:bg-primary-hover text-white rounded-xl text-sm font-medium transition-colors"
            >
              설치
            </button>
          </div>
