'use client';

import { useState, useEffect } from 'react';
import Link from 'next/link';
import { useAuthContext } from '@/context/AuthContext';

// Prevent SSR for this page
export const runtime = 'edge';

interface AuthResponse {
  user?: {
    email: string;
    name: string;
    picture: string;
    isAdmin: boolean;
  };
  error?: string;
}

export default function AuthTestPage() {
  const { user, isAuthenticated, isAdmin, isLoading, login, logout } = useAuthContext();
  const [apiResponse, setApiResponse] = useState<AuthResponse | null>(null);
  const [apiError, setApiError] = useState<string | null>(null);
  const [isChecking, setIsChecking] = useState(false);

  const checkAuthApi = async () => {
    setIsChecking(true);
    setApiError(null);
    setApiResponse(null);

    try {
      const response = await fetch('/api/auth/me');
      const data = await response.json();
      setApiResponse(data);
    } catch (err) {
      setApiError(err instanceof Error ? err.message : 'Failed to check auth');
    } finally {
      setIsChecking(false);
    }
  };

  useEffect(() => {
    checkAuthApi();
  }, [isAuthenticated]);

  const handleLogin = () => {
    login();
  };

  const handleLogout = async () => {
    await logout();
    checkAuthApi();
  };

  return (
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          <Link href="/test" className="text-foreground/60 hover:text-foreground">
            ← Back
          </Link>
          <h1 className="text-2xl font-bold text-foreground">Google Login Test</h1>
        </div>

        {/* Auth Context State */}
        <div className="bg-card-bg border border-card-border rounded-lg p-6 mb-6">
          <h2 className="text-lg font-semibold text-foreground mb-4">Auth Context State</h2>
          <div className="space-y-2 text-sm font-mono">
            <div className="flex gap-2">
              <span className="text-foreground/60">isLoading:</span>
              <span className="text-foreground">{String(isLoading)}</span>
            </div>
            <div className="flex gap-2">
              <span className="text-foreground/60">isAuthenticated:</span>
              <span className={isAuthenticated ? 'text-green-500' : 'text-red-500'}>
                {String(isAuthenticated)}
              </span>
            </div>
            <div className="flex gap-2">
              <span className="text-foreground/60">isAdmin:</span>
              <span className={isAdmin ? 'text-green-500' : 'text-yellow-500'}>
                {String(isAdmin)}
              </span>
            </div>
            {user && (
              <>
                <div className="flex gap-2">
                  <span className="text-foreground/60">user.email:</span>
                  <span className="text-foreground">{user.email}</span>
                </div>
                <div className="flex gap-2">
                  <span className="text-foreground/60">user.name:</span>
                  <span className="text-foreground">{user.name}</span>
                </div>
                <div className="flex gap-2 items-center">
                  <span className="text-foreground/60">user.picture:</span>
                  <img
                    src={user.picture}
                    alt="Profile"
                    className="w-8 h-8 rounded-full"
                  />
                </div>
              </>
            )}
          </div>
        </div>

        {/* API Response */}
        <div className="bg-card-bg border border-card-border rounded-lg p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-foreground">/api/auth/me Response</h2>
            <button
              onClick={checkAuthApi}
              disabled={isChecking}
              className="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 disabled:opacity-50"
            >
              {isChecking ? 'Checking...' : 'Refresh'}
            </button>
          </div>

          {apiError && (
            <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-lg text-red-500 mb-4">
              Error: {apiError}
            </div>
          )}

          {apiResponse && (
            <pre className="p-4 bg-background rounded-lg overflow-x-auto text-xs font-mono text-foreground/80">
              {JSON.stringify(apiResponse, null, 2)}
            </pre>
          )}
        </div>

        {/* Login/Logout Actions */}
        <div className="bg-card-bg border border-card-border rounded-lg p-6">
          <h2 className="text-lg font-semibold text-foreground mb-4">Actions</h2>
          <div className="flex gap-4">
            {!isAuthenticated ? (
              <button
                onClick={handleLogin}
                className="px-6 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 flex items-center gap-2"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  <path
                    fill="currentColor"
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                  />
                  <path
                    fill="currentColor"
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  />
                  <path
                    fill="currentColor"
                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  />
                  <path
                    fill="currentColor"
                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  />
                </svg>
                Login with Google
              </button>
            ) : (
              <button
                onClick={handleLogout}
                className="px-6 py-3 bg-red-500 text-white rounded-lg hover:bg-red-600"
              >
                Logout
              </button>
            )}
          </div>

          {isAuthenticated && !isAdmin && (
            <div className="mt-4 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg text-yellow-600 dark:text-yellow-400">
              You are logged in but not an admin. Add your email to the allowedUploaders list in metadata.json to get admin access.
            </div>
          )}
        </div>
      </div>
    </div>
  );
}
