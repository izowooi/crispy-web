...
faithful port (`approach-4-faithful-port/web`) is being measured against, and
...
- 2026-05-29 ai-throw fix (TDD):
  ...
  (a) `web/src/core/factories.ts` dispatch only invoked function-typed
  listeners, dropping the object listener that `Game.dolevel` registers via
  `addEventListener("throwball", this)`; and (b) `web/src/main.ts` titles
  ...
  Tests added at `web/tests/unit/AI.throwIntegration.test.ts` (2 tests):
  ...
  is correct.
  Fixes applied:
   - `web/src/core/factories.ts`: `dispatch()` now handles both function
   ...
   - `web/src/main.ts`: titles shim now models the SWF DefineSprite_110
...
- 2026-05-29 presentation fixes (TDD) — user feedback: "death frame flickers,
  ...
  1. DEATH FLICKER. `web/src/main.ts` fed a single MONOTONIC per-dudie tick to
     ...
     script uses `_currentframe`/GotoFrame2 to hold). Fix:
      - `web/src/render/Animation.ts`: added `HOLD_LAST_POSES`
        (dead/down/hit/midrecover/yea/hitdazed). For those, `frameForState`
        clamps `first + min(tick, length-1)` instead of wrapping.
      - `web/src/render/PoseClock.ts` (new): per-dudie animation clock that
  ...
  2. CHARGE METER. FFDec had exported the red dudie frames from the PARENT
     ...
     permanent selection ring AND a frozen meter tick that never charged. Fix:
      - Re-extracted clean red body frames: removed chid 8 + chid 19 with
        ...
        re-trimmed to `web/public/assets/sprites/red/*.png` + `index.json`
      ...
      - `web/src/core/meter.ts` (new): `chargeFrame(heldMs)` / `chargeForce`
...
- 2026-05-29 round-3 polish (3 user-reported issues) + extended campaign:
  1. AI STEP SOUND too loud/odd. `web/src/core/factories.ts` hard-coded the
  ...
  3. TRACKPAD DRAG never threw on finger-lift; the throw only fired on the
     ...
     `web/src/input/Input.ts` to Pointer Events + `setPointerCapture` +
  ...
  5 NEW levels 10..14 (`EXTRA_LEVELS` in `web/src/core/levelConfig.ts`,
