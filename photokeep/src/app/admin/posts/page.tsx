...
function SortablePostItem({ post, reordering, deleting, onEdit, onDelete }: SortablePostItemProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    >
      ...
      <button
        ...
        {...listeners}
        disabled={reordering}
        className={`flex-shrink-0 touch-none p-1 text-muted ${
          reordering ? 'opacity-30 cursor-not-allowed' : 'cursor-grab active:cursor-grabbing hover:text-foreground'
        }`}
